...
namespace FaceFuncs{
	...
	void read_csv(const string& filename, vector<Mat>& images, vector<int>& labels, vector<string>& names, char separator = ';') {
	...
	tuple<cv::Point, int, string> pointReturn(int saved_model, Mat inputFrame, int j){
		string it;
		...
		return make_tuple(CenterPoint, j, it);
